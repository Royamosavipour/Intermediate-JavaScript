...
let $ = document;
let inputElem = $.querySelector("#amount");
...
button.addEventListener("click", function (e) {
  ...
  if (
    ...
    inputElemValue > 9 ||
    inputElemValue === "" ||
    isNaN(inputElemValue)
  ) {
    result.innerHTML = dummyText[randomText];
  } else {
    let inputslice = dummyText.slice(0, inputElemValue);
    let newslice = inputslice.map(function (item) {
      return "<p>" + item + "</p>";
    }).join('')


    result.innerHTML=newslice
  }
