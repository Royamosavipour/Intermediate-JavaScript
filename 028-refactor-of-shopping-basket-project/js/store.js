...
function basketProductsGenerator(userBasketArray) {
  ...
  userBasketArray.forEach(function (product) {
    ...
    // bastekProductsContainer.insertAdjacentHTML(
    //   'beforeend',
    //   '<div class="cart-row"><div class="cart-item cart-column"><img src="' +
    //     product.img +
    //     '" width="100" height="100" class="cart-item-image"><span class="cart-item-title">' +
    //     product.title +
    //     '</span></div><span class="cart-price cart-column">' +
    //     product.price +
    //     '</span><div class="cart-quantity cart-column"><input class="cart-quantity-input" type="number" onchange="updateProductCount(' +
    //     product.id,
    //   `${basketProductInput.value})"><button class="btn btn-danger" onclick="removeProductFromBasket(${product.id})">Remove</button></div></div>`
    // );
  });
}
